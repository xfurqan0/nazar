...
![A twenty-six second tour of the Nazar canvas: five session cards with their subagent trees drawn underneath, a banner and a Needs you badge for the one session waiting on a permission prompt, the panel behind the badge saying which one to answer first, a hover card with the model and the exact token counts, the History panel opening a past run as a frozen tree, the Midnight theme repainting the whole canvas, and a Codex thread with no subagents and no cost](docs/media/tour.gif)
...
**More screenshots:** [docs/screenshots/README.md](docs/screenshots/README.md) is the gallery — every picture this README used to carry, under the feature it belongs to, and one command that retakes them all.
