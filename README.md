...
![The Nazar canvas: four sessions with their subagent trees. The working one has a green ring and reads working, the resting one a blue ring reading idle, the one whose process stopped answering a dashed grey ring reading unknown, and the one waiting on a permission prompt is the single card with a coloured border, in amber, reading permission prompt](docs/screenshots/wp4d-activity-dark.png)
